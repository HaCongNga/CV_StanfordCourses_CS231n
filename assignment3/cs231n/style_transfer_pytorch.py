...
def content_loss(content_weight, content_current, content_original):
    ...
    reshaped_target = content_original.reshape(content_original.shape[1], -1)
    reshaped_current = content_current.reshape(content_current.shape[1], -1)
    loss = content_weight * (((reshaped_current - reshaped_target) **2).sum())
    return loss
...
def gram_matrix(features, normalize=True):
    ...
    gram_matr = torch.bmm(reshaped_features, reshaped_features.transpose(1,2))
    if normalize :
      gram_matr /= (H*W*C)
    return gram_matr
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    - style_layers: List of layer indices into feats giving the layers to include in the
      style loss. # Specify which layer is included
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    """
    grad_matrices = gram_matrix(feats, normalize=True)
    style_loss = (((grad_matrices - style_targets) ** 2)*style_weights).sum()
    """
    num_layers = len(style_layers)
    style_loss = 0
    for i in range(num_layers) :
      L = style_layers[i]
      grad_matrices = gram_matrix(feats[L], normalize=True)
      style_loss += (((grad_matrices - style_targets[i]) ** 2)*style_weights[i]).sum()
    return style_loss
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def tv_loss(img, tv_weight):
    ...
    # Create "diff" row
    below_matrix = img[:,:,1:,:]
    above_matrix = img[:, :,0:-1,:]
    diff_row = above_matrix-below_matrix 
    # Create "diff" column
    right_matrix = img[:,:,:,1:]
    left_matrix = img[:, :,:,0:-1]
    diff_col = right_matrix-left_matrix 
    loss = tv_weight * ((diff_row ** 2).sum() + (diff_col**2).sum())
    return loss
